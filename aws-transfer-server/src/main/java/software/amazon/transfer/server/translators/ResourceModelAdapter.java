...
public final class ResourceModelAdapter {
    ...
    public static final String DEFAULT_IDENTITY_PROVIDER_TYPE =
            IdentityProviderType.SERVICE_MANAGED.name();
    ...
    public static final List<String> DEFAULT_PROTOCOLS =
            Collections.singletonList(Protocol.SFTP.name());
    ...
    private static void setPreviousTags(
        ...
        tagsMap.putAll(
                Optional.ofNullable(
                                Translator.translateTagListToTagMap(
                                        previousResourceModel.getTags()))
                        .orElseGet(HashMap::new));
        tagsMap.putAll(
                Optional.ofNullable(request.getPreviousSystemTags()).orElseGet(HashMap::new));
